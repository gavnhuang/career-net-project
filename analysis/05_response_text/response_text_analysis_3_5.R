...
pdf("plots/response_text/response_text_analysis_3_5.pdf", width = 12, height = 8)
print(p1)
print(p2)
print(p3)
print(p4)
...
write_csv(word_tfidf,   "outputs/response_text/distinctive_words_3_5.csv")
write_csv(bigram_tfidf, "outputs/response_text/distinctive_bigrams_3_5.csv")
...
cat("\n====================================================\n")
cat("RESPONSE TEXT ANALYSIS (> 3.5 vs <= 3.5) COMPLETE\n")
cat("====================================================\n\n")
cat("Composite > 3.5: ", scales::comma(n_high), "\n")
cat("Composite <= 3.5:", scales::comma(n_low),  "\n\n")

cat("Most distinctive words — > 3.5 responses:\n")
word_tfidf %>%
...
cat("\nMost distinctive words — <= 3.5 responses:\n")
word_tfidf %>%
  filter(quality_group == "<= 3.5") %>%
  arrange(desc(tf_idf)) %>%
  transmute(word, n, pct = round(pct, 3), tf_idf = round(tf_idf, 5)) %>%
...
cat("\nMost distinctive phrases — > 3.5 responses:\n")
bigram_tfidf %>%
  filter(quality_group == "> 3.5") %>%
  arrange(desc(tf_idf)) %>%
