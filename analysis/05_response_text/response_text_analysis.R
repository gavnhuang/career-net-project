...
all_data <- bind_rows(
  ...
  mutate(
    ...
    response_text     = str_remove_all(coalesce(answer_body, ""), "<.*?>")
  ) %>%
  filter(
    !is.na(completeness_num), !is.na(coherency_num), !is.na(correctness_num),
    str_trim(response_text) != ""
  ) %>%
  mutate(quality_group = ifelse(composite_quality > 3.0, "> 3.0", "<= 3.0"))

n_high <- sum(all_data$quality_group == "> 3.0")
n_low  <- sum(all_data$quality_group == "<= 3.0")
...
tokens <- all_data %>%
  select(quality_group, response_text) %>%
...
bigrams <- all_data %>%
  select(quality_group, response_text) %>%
  unnest_tokens(bigram, response_text, token = "ngrams", n = 2) %>%
...
word_log_odds <- word_freq %>%
  ...
  filter(`> 3.0` + `<= 3.0` >= 10) %>%
  mutate(
    p_high   = (`> 3.0`  + 0.5) / (n_high + 0.5),
    p_low    = (`<= 3.0` + 0.5) / (n_low  + 0.5),
...
group_colors <- c("> 3.0" = "#2C7BB6", "<= 3.0" = "#E66101")
...
p1 <- word_freq %>%
  ...
  labs(
    title    = "Most Common Words by Response Quality Group",
    subtitle = paste0(
      "Top 20 words by frequency | Stop words removed | ",
      "> 3.0 n = ", scales::comma(n_high), " | <= 3.0 n = ", scales::comma(n_low)
    ),
...
p2 <- word_tfidf %>%
  ...
  facet_wrap(~ quality_group, scales = "free_y") +
  labs(
    title    = "Most Distinctive Words by Response Quality Group",
...
p3 <- bigram_tfidf %>%
  ...
  facet_wrap(~ quality_group, scales = "free_y") +
  labs(
    title    = "Most Distinctive Two-Word Phrases by Response Quality Group",
...
dir.create("plots/response_text",   recursive = TRUE, showWarnings = FALSE)
dir.create("outputs/response_text", recursive = TRUE, showWarnings = FALSE)

pdf("plots/response_text/response_text_analysis.pdf", width = 12, height = 8)
print(p1)
print(p2)
print(p3)
print(p4)
...
write_csv(word_tfidf,   "outputs/response_text/distinctive_words.csv")
write_csv(bigram_tfidf, "outputs/response_text/distinctive_bigrams.csv")
...
cat("\n====================================================\n")
cat("RESPONSE TEXT ANALYSIS (> 3.0 vs <= 3.0) COMPLETE\n")
cat("====================================================\n\n")
cat("Composite > 3.0: ", scales::comma(n_high), "\n")
cat("Composite <= 3.0:", scales::comma(n_low),  "\n\n")

cat("Most distinctive words — > 3.0 responses:\n")
word_tfidf %>%
  filter(quality_group == "> 3.0") %>%
  arrange(desc(tf_idf)) %>%
  transmute(word, n, pct = round(pct, 3), tf_idf = round(tf_idf, 5)) %>%
  print(n = 20)

cat("\nMost distinctive words — <= 3.0 responses:\n")
word_tfidf %>%
  filter(quality_group == "<= 3.0") %>%
  arrange(desc(tf_idf)) %>%
  transmute(word, n, pct = round(pct, 3), tf_idf = round(tf_idf, 5)) %>%
...
cat("\nMost distinctive phrases — > 3.0 responses:\n")
bigram_tfidf %>%
  filter(quality_group == "> 3.0") %>%
  arrange(desc(tf_idf)) %>%
