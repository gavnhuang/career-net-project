...
clean_careernet_data <- function(data) {
  data %>%
    mutate(
      ...
      response_length = str_count(answer_body, "\\S+"),
...
remove_length_outliers <- function(data) {
  q1 <- quantile(data$response_length, 0.25)
  q3 <- quantile(data$response_length, 0.75)
  iqr_value <- IQR(data$response_length)
  ...
  data %>%
    filter(
      response_length >= lower_bound,
      response_length <= upper_bound
    )
}
...
get_regression <- function(outcome_col, data, label) {
  model <- lm(reformulate("response_length", outcome_col), data = data)
...
make_response_length_plot <- function(data, y_variable, title, y_label, stats_label) {
  ggplot(data, aes(x = response_length, y = .data[[y_variable]])) +
...
run_response_length_analysis <- function(data, analysis_name, plot_file, output_folder) {
  ...
  rating_summary <- no_outliers %>%
    ...
    summarise(
      mean_words   = round(mean(response_length), 1),
      median_words = round(median(response_length), 1),
      responses    = n(),
      .groups = "drop"
    )

  correlation_summary <- imap_dfr(outcomes, function(col, label) {
    get_correlation(no_outliers$response_length, no_outliers[[col]], label)
  })
  ...
  plots <- imap(outcomes, function(col, label) {
    make_response_length_plot(
      no_outliers,
      col,
      paste(analysis_name, "Response Length vs", label),
      paste(label, "Rating"),
  ...
  cat("Average Response Length by Rating:\n")
