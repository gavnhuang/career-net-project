...
pdf("plots/prompt_text/prompt_text_analysis.pdf", width = 12, height = 8)
print(p1)
print(p2)
print(p3)
print(p4)
...
write_csv(word_tfidf,   "outputs/prompt_text/distinctive_words.csv")
write_csv(bigram_tfidf, "outputs/prompt_text/distinctive_bigrams.csv")
...
cat("\n====================================================\n")
cat("PROMPT TEXT ANALYSIS (> 3.0 vs <= 3.0) COMPLETE\n")
cat("====================================================\n\n")
cat("Composite > 3.0: ", scales::comma(n_high), "\n")
cat("Composite <= 3.0:", scales::comma(n_low),  "\n\n")

cat("Most distinctive words — > 3.0 prompts:\n")
word_tfidf %>%
...
cat("\nMost distinctive words — <= 3.0 prompts:\n")
word_tfidf %>%
  filter(quality_group == "<= 3.0") %>%
  arrange(desc(tf_idf)) %>%
  transmute(word, n, pct = round(pct, 3), tf_idf = round(tf_idf, 5)) %>%
...
cat("\nMost distinctive phrases — > 3.0 prompts:\n")
bigram_tfidf %>%
  filter(quality_group == "> 3.0") %>%
  arrange(desc(tf_idf)) %>%
