...
clean_careernet_data <- function(data) {
  data %>%
    mutate(
      ...
      prompt_length  = str_count(question_clean, "\\S+"),
...
remove_length_outliers <- function(data) {
  q1        <- quantile(data$prompt_length, 0.25)
  q3        <- quantile(data$prompt_length, 0.75)
  iqr_value <- IQR(data$prompt_length)

  data %>%
    filter(
      prompt_length >= q1 - 1.5 * iqr_value,
      prompt_length <= q3 + 1.5 * iqr_value
    )
}
...
get_regression <- function(outcome_col, data, label) {
  model         <- lm(reformulate("prompt_length", outcome_col), data = data)
...
make_prompt_length_plot <- function(data, y_variable, title, y_label, stats_label) {
  ggplot(data, aes(x = prompt_length, y = .data[[y_variable]])) +
    geom_point(alpha = 0.15) +
    geom_smooth(method = "lm", se = TRUE) +
    labs(
      title    = title,
      subtitle = "Blue line = regression trend line | Shaded band = uncertainty",
      x        = "Prompt Length in Words",
      y        = y_label,
      caption  = stats_label
    ) +
...
run_prompt_length_analysis <- function(data, analysis_name, plot_file, output_folder) {
  ...
  rating_summary <- no_outliers %>%
    ...
    summarise(
      mean_words   = round(mean(prompt_length), 1),
      median_words = round(median(prompt_length), 1),
      responses    = n(),
      .groups = "drop"
    )

  correlation_summary <- imap_dfr(outcomes, function(col, label) {
    get_correlation(no_outliers$prompt_length, no_outliers[[col]], label)
  })
  ...
  plots <- imap(outcomes, function(col, label) {
    make_prompt_length_plot(
      no_outliers,
      col,
      paste(analysis_name, "Prompt Length vs", label),
      paste(label, "Rating"),
  ...
  cat("Average Prompt Length by Rating:\n")
